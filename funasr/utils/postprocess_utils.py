...
def rich_transcription_postprocess(rec_result, min_emo=4):
    timestamp, text = rec_result["timestamp"], rec_result["text"]
    ...
    emo_event_set = set(emo_set | event_set)
    ...
    for i in range(1, len(s_list)):
        ...
        cur_text = s_list[i].strip().lstrip()
        cur_text = [char for char in cur_text if char not in emo_event_set]
        cur_text = "".join(cur_text)
        cur_emo = get_emo(s_list[i].strip().lstrip())
        if cur_emo == "😊":
            cur_emo = 1
        elif cur_emo == "😡" and len(cur_text) > min_emo:
            cur_emo = -1
        else:
            cur_emo = 0
        res.append({"text": cur_text, "emo": cur_emo})
    for idx in range(len(res)):
        res[idx]["timestamp"] = timestamp[idx]
    return res
